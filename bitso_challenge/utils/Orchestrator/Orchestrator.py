...
class Orchestrator:
    ...
    def prepare_data_for_transform(self) -> None:
        try:
            logger.info("Preparing data for transformation")
            logger.info("Transforming Bitso data")
            self.bitso = self.transformer.transform_bitso_data(self.exchanges)
            logger.info("Reading market USD volume")
            ...
            logger.info("Successfully prepared data for transformation")
        except Exception as e:
            logger.error("Error preparing data for transformation: %s", e)
    ...
    def run(self) -> None:
        try:
            self.fetch_data()
            self.prepare_data_for_transform()
            self.transform_data()
            self.write_data()
